...
def decode_sequence(encoder_model, decoder_model, testX, test_ith, test_ith_str, test_word, max_seq_len, word2idx):
    encoder_out, _ = encoder_model.predict_on_batch([testX, test_ith_str, test_word])
    _, state = encoder_model.predict_on_batch([testX[:, :2], test_ith_str, test_word])
    encoder_state = state.copy()
    ...
    for i in range(max_seq_len):
        decoder_out, state = decoder_model.predict_on_batch([target_seq[:, i:i+1], encoder_out[:, -1], state, test_ith, test_word])
