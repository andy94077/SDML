...
def decode_sequence(encoder_model, decoder_model, testX, test_ith, test_ith_str, test_word, max_seq_len, word2idx):
    encoder_out, state = encoder_model.predict_on_batch([test_ith_str, test_word])
...
if __name__ == '__main__':
    ...
    if submit:
        with open(submit, 'r') as t:
            ...
            batch_size = 1024
            encoder_vec_and_state = np.concatenate(encoder_model.predict([testX, test_ith_str, test_word], batch_size=batch_size), axis=1)
            np.save(submit[submit.rfind('/')+1:submit.rfind('.')]+'.npy', encoder_vec_and_state)
